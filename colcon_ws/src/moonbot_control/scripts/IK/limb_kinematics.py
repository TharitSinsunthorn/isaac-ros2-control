...
class InvKinematics():
    ...
    def get_RF_joint_angles(self, coord, eularAng = [0,0,0]):
        ...
        coord_ = np.dot((np.dot((coord + translate_RF), self.rotMat(eularAng)) - translate_RF), self.rotMat([0,0,7*math.pi/4]))
    ...
    def get_LF_joint_angles(self, coord, eularAng = [0,0,0]):
        ...
        coord_ = np.dot((np.dot((coord + translate_LF), self.rotMat(eularAng)) - translate_LF), self.rotMat([0,0,1*math.pi/4]))
        # check singularity of the legs
        if self.is_singularity(coord_):
    ...
    def get_LR_joint_angles(self, coord, eularAng = [0,0,0]):
        ...
        coord_ = np.dot((np.dot((coord + translate_LR), self.rotMat(eularAng)) - translate_LR), self.rotMat([0,0,3*math.pi/4]))
        # check singularity of the legs
        if self.is_singularity(coord_):
    ...
    def get_RR_joint_angles(self, coord, eularAng = [0,0,0]):
        ...
        coord_ = np.dot((np.dot((coord + translate_RR), self.rotMat(eularAng)) - translate_RR), self.rotMat([0,0,5*math.pi/4]))
        # check singularity of the legs
        if self.is_singularity(coord_):
